...
export default function CardsPage() {
	...
	const copyNfcUrl = (cardId: string) => {
		const url = `${window.location.origin}/n/${cardId}`;
			...
			<div className="mx-auto max-w-5xl space-y-8">
				...
				<div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
					<div>
						...
						<p className="text-white/60">Vincule chips físicos aos seus perfis. Altere o destino sem regravar o cartão.</p>
					</div>
					<Dialog open={addOpen} onOpenChange={setAddOpen}>
						...
						<DialogContent>
							<DialogHeader>
								...
								<DialogDescription>
									Registre um novo chip NFC e vincule a um perfil. A URL gerada deve ser gravada no chip.
								</DialogDescription>
							</DialogHeader>
				...
				<div className="rounded-xl border border-violet-500/30 bg-violet-500/10 p-4 flex items-start gap-3">
					...
					<div className="text-sm text-white/80">
						<strong className="text-violet-300">Como funciona:</strong> Cada cartão tem uma URL única (<code className="text-violet-300">/n/[id]</code>) para gravar no chip NFC. Quando alguém aproxima o celular, o sistema redireciona automaticamente para o perfil vinculado. Troque o perfil quando quiser — <strong>sem regravar o chip.</strong>
					</div>
				</div>
				...
				{cards.length === 0 && (
					<Card className="bg-white/5 border-white/10">
						<CardContent className="flex flex-col items-center justify-center p-16 text-center">
							...
							<p className="mb-6 text-white/60">Registre seu primeiro chip NFC para começar a gerenciar.</p>
						</CardContent>
					</Card>
				)}
						...
						>
							...
							<CardContent className="space-y-4">
								...
								<div className="space-y-1.5">
									<label className="text-xs font-medium text-white/50 uppercase tracking-wider">Alterar Destino</label>
									<Select
								...
								<div className="flex items-center gap-2">
									<code className="flex-1 truncate rounded bg-black/30 px-2 py-1 text-xs text-violet-300">
										{window?.location?.origin ?? ""}/n/{card.id}
									</code>
									<Button
										size="sm"
										variant="ghost"
